...
if ~isnumeric(V)
    if strcmp(V,'noise')
        ...
        V = mvnrnd(zeros(1,model.x_dim), model.Q, size(Xd,2))';
    elseif strcmp(V,'noiseless')
        %V= zeros(size(model.B,1),size(Xd,2));
        V = zeros(model.x_dim,1);
        V = repmat(V, 1, size(Xd,2));
    end
end
...
if isempty(Xd)
    ...
    %--  x/y pos/vel
    X(1,:)= Xd(1,:)+ a.*Xd(3,:)- b.*Xd(4,:);
    X(3,:)= cos_omega_T.*Xd(3,:)- sin_omega_T.*Xd(4,:);
    X(2,:)= b.*Xd(3,:) + Xd(2,:)+ a.*Xd(4,:);
    X(4,:)= sin_omega_T.*Xd(3,:)+ cos_omega_T.*Xd(4,:);
    %-- turn rate
    X(5,:)= Xd(5,:);
    %-- add scaled noise 
    %X= X+ model.B2*V;

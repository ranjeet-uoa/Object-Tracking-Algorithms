function [qz_update,m_update,P_update] = ckf_update_multiple(z,model,m,P)

plength= size(m,2);
zlength= size(z,2);

qz_update= zeros(plength,zlength);
m_update = zeros(model.x_dim,plength,zlength);
P_update = zeros(model.x_dim,model.x_dim,plength);

for idxp=1:plength
        [qz_temp,m_temp,P_temp] = ckf_update_single(z,model,m(:,idxp),P(:,:,idxp));
       qz_update(idxp,:)   = qz_temp;
        m_update(:,idxp,:) = m_temp;
        P_update(:,:,idxp) = P_temp;
end

function [qz_temp,m_temp,P_temp] = ckf_update_single(z,model,m,P)

[X_ckf,wt]= ct( m, P);
Z_pred= gen_observation_fn( model, X_ckf, 'noiseless' );

eta= Z_pred*wt(:);

S_temp= Z_pred- repmat(eta,[1 length(wt)]);

S= S_temp*diag(wt)*S_temp' + model.R;

Vs= chol(S); det_S= prod(diag(Vs))^2; inv_sqrt_S= inv(Vs); iS= inv_sqrt_S*inv_sqrt_S';

G_temp= X_ckf- repmat(m,[1 length(wt)]);
G= G_temp*diag(wt)*S_temp';

K = G * iS; 
%K  = G * pinv(S);

qz_temp = exp(-0.5*size(z,1)*log(2*pi) - 0.5*log(det_S) - 0.5*dot(z-repmat(eta,[1 size(z,2)]),iS*(z-repmat(eta,[1 size(z,2)]))))';
m_temp = repmat(m,[1 size(z,2)]) + K*(z-repmat(eta,[1 size(z,2)]));
P_temp = P- G*iS*G';
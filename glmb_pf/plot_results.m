...
legend([hline2 hline1 hlined],'Estimates','True tracks','Measurements', 'Location','best');
...
subplot(3,1,1); plot(1:meas.K,ospa_vals(:,1),'k'); grid off; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Dist');
subplot(3,1,2); plot(1:meas.K,ospa_vals(:,2),'k'); grid off; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Loc');
subplot(3,1,3); plot(1:meas.K,ospa_vals(:,3),'k'); grid off; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 ospa_c]); ylabel('OSPA Card');
xlabel('Time');
...
subplot(3,1,1);
for i = 1:length(win_len)
    plot(1:truth.K,ospa2_cell{i}(1,:),'k'); grid off; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 cutoff]); ylabel('OSPA$^{(2)}$ Dist','interpreter','latex');
...
subplot(3,1,2);
for i = 1:length(win_len)
    plot(1:truth.K,ospa2_cell{i}(2,:),'k'); grid off; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 cutoff]); ylabel('OSPA$^{(2)}$ Loc','interpreter','latex');
    windowlengthlabels{i} = ['$L_w = ' int2str(win_len(i)) '$'];
end

subplot(3,1,3);
for i = 1:length(win_len)
    plot(1:truth.K,ospa2_cell{i}(3,:),'k'); grid off; set(gca, 'XLim',[1 meas.K]); set(gca, 'YLim',[0 cutoff]); ylabel('OSPA$^{(2)}$ Card','interpreter','latex');
    windowlengthlabels{i} = ['$L_w = ' int2str(win_len(i)) '$'];
end
...
grid on;
legend(gca,'True','Estimated', 'Location','best'); legend boxoff

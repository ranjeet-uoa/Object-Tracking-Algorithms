function est = run_filter_pf(model,meas)
...
filter.npt= 20000;                   %number of particles per track
filter.nth= filter.npt/3;            %threshold on effective number of particles before resampling
filter.sir_flag =1;                  %To enable the SIR (resampling at each step)
...
for tabidx= 1:length(glmb_clean.tt)
    ...
    if (neffsamptemp < filter.nth) || filter.sir_flag
        xtemptemp= glmb_clean.tt{tabidx}.x;
        wtemptemp= glmb_clean.tt{tabidx}.w;
        %rspidx= randsample(length(wtemptemp),filter.npt,true,wtemptemp); %choose 'Multinomial' for speed
        rspidx= resample(wtemptemp,filter.npt,'sys');  % choose either 'sys' (faster) or 'Systematic' for accuracy

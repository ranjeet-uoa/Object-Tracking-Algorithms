...
for tabsidx=1:length(glmb_update.tt)
    [mtemp_predict,Ptemp_predict]= ckf_predict_multiple(model,glmb_update.tt{tabsidx}.m,glmb_update.tt{tabsidx}.P);      %cubature kalman prediction for GM
...
if filter.gate_flag
    for tabidx=1:length(glmb_predict.tt)
        glmb_predict.tt{tabidx}.gatemeas= gate_meas_ckf_idx(meas.Z{k},filter.gamma,model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);
    end
else
    for tabidx=1:length(glmb_predict.tt)
...
for tabidx= 1:length(glmb_predict.tt)
    for emm= glmb_predict.tt{tabidx}.gatemeas
            ...
            [qz_temp,m_temp,P_temp] = ckf_update_multiple(meas.Z{k}(:,emm),model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);   %cubature kalman update for this track and this measurement
...
for t=1:length(est.T)
    ...
    for u=1:length(tah)
        [m,P] = ckf_predict_multiple(model,m,P);
        k= ks+u-1;
        emm= tah(u);
        if emm > 0
            [qz,m,P] = ckf_update_multiple(meas.Z{k}(:,emm),model,m,P);
            w= qz.*w+eps;
            w= w/sum(w);
        end
